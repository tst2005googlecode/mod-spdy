...
namespace {
void SplitStringPieceToVector(StringPiece, char*, std::vector<StringPiece>*, bool) {
  CHECK(false);
}
}
...
void BalsaFrame::ProcessFirstLine(const char* begin, const char* end) {
  ...
  if (!ParseHTTPFirstLine(begin,
                          ...
                          headers_,
                          &last_error_)) {
    parse_state_ = BalsaFrameEnums::PARSE_STATE_ERROR;
...
void BalsaFrame::ProcessContentLengthLine(
  ...
  *length = 0;
  while (value_begin < line_end) {
    ...
    if (*length > kMaxDiv10 ||
		(std::numeric_limits<size_t>::max() - length_x_10) < static_cast<size_t>(c)) {
...
void BalsaFrame::ProcessTransferEncodingLine(HeaderLines::size_type line_idx) {
  ...
  if ((value_length == 7) &&
	  !base::strncasecmp(value_begin, "chunked", 7)) {
    headers_->transfer_encoding_is_chunked_ = true;
  } else if ((value_length == 8) &&
	  !base::strncasecmp(value_begin, "identity", 8)) {
...
void ProcessChunkExtensionsGoogle3(const char* input, size_t size,
                                   BalsaHeaders* extensions) {
  std::vector<StringPiece> key_values;
    ...
    for (HeaderLines::size_type i = 0; i < header_lines_size; ++i) {
      ...
      if (c == 'c' || c == 'C') {
        if ((key_len == kContentLengthSize) &&
			0 == base::strncasecmp(key_begin, kContentLength, kContentLengthSize)) {
        ...
        if ((key_len == kTransferEncodingSize) &&
			0 == base::strncasecmp(key_begin, kTransferEncoding,
...
size_t BalsaFrame::ProcessHeaders(const char* message_start,
        ...
        while (message_current < message_end_m16) {
          ...
          LOG(INFO) << "is_request_: " << is_request_;
            ...
            if (parse_state_ == BalsaFrameEnums::MESSAGE_FULLY_READ)
              goto process_lines;
            else if (parse_state_ == BalsaFrameEnums::PARSE_STATE_ERROR)
              goto bottom;
          }
 ...
 process_lines:
    ...
    ProcessHeaderLines();
    if (parse_state_ == BalsaFrameEnums::PARSE_STATE_ERROR) {
      goto bottom;
    }
          ...
          while (current < end) {
            ...
            if (addition >= 0) {
              ...
              if ((chunk_length_remaining_ > kMaxDiv16) ||
                  (std::numeric_limits<size_t>::max() - length_x_16) < static_cast<size_t>(addition)) {
