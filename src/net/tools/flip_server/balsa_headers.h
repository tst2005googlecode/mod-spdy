...
class BalsaBuffer {
 public:
  ...
  void Swap(BalsaBuffer* b) {
	...
	std::swap(can_write_to_contiguous_buffer_, b->can_write_to_contiguous_buffer_);
...
class BalsaHeaders {
 public:
  ...
  typedef std::vector<StringPiece> HeaderTokenList;
  ...
  class iterator_base {
   ...
   protected:
    ...
    void increment() {
      ...
      // and the end of the std::vector of HeaderLineDescriptions.
    ...
    void decrement() {
      ...
      // and the beginning of the std::vector of HeaderLineDescriptions.
      // TODO(fenix): refactor this list so that we don't have to do
      // linear scanning through skipped headers (and this condition is
      // then unnecessary)
 ...
 public:
  ...
  // values into the std::vector 'out', in top-to-bottom order.
  ...
  // all, out will not be changed. We do not clear the std::vector out
  // before adding new entries. If there are header lines with matching
  // key but empty value then they are also added to the std::vector out.
  ...
  //    "key1:v2\r\n"
  //
  //  std::vector out is initially: ["foo"]
  //  std::vector out after GetAllOfHeader("key1", &out) is:
  // ["foo", "v1", "", "", "v2", "v1", "v2"]

  void GetAllOfHeader(const StringPiece& key, std::vector<StringPiece>* out) const;
  ...
  // you don't need the intermediate std::vector<StringPiece>.
  void GetAllOfHeaderAsString(const StringPiece& key, std::string* out) const;
